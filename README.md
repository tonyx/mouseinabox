...
A box is modelled like like a rectangular grid with cells having coordinates like (0,0)  (south west) and (N,M) (north east).

The user can set N and N.

The user can place a mouse in the box setting its position (i.e the cell) and its orientation: South, North, East, West. 
...
For instance, if the mouse is oriented to east and its position is (x,y) then its position after a forward command will be (x+1, y).

The cells of the box can be free or locked. At the beginning all the cells are free.
...
For instance, the sequence of characters 53 indicate that M is 5 and N is 3.
...
Some examples:
set the box dimensions as 5 x 3, then place the first mouse in the position 11 oriented to East and
give it the commands RFRFRFRF
second and third mouse will be give similar commands
```
53
...
In the precedent part we have seen what is needed to make the mouse move in the box according to the commands.
...
everything updated. The new world will have as list of command the "tail" of the
...
testing based on mock) we have a IOConnectors module where I defined OutputStream and InputStream as interfaces, 
